...
import chroma from 'chroma-js'; //*ADDED*
...
const numBallsInput   = document.querySelector('#num-balls');  // *ADDED*
const rightProbInput  = document.querySelector('#right-prob');  // *ADDED*
...
function easeOutQuad(t) { return t*(2-t); } // *ADDED* the given function from problem set 5 instructions
function easeInQuad(t) { return t*t; } // *ADDED* the given function from problem set 5 instructions

// This function is taken from the problem 5 instructions and *MODIFIED*
function animate(circle, to, duration, any, ease) {
    // function ease(t) { return t; }
    return new Promise((resolve, reject) => {
      const from = parseFloat(circle.getAttribute(any));
      const animationStarted = Date.now();
      
      function step() {
        const pct = (Date.now() - animationStarted) / duration;
        const pos = ease(pct);
        const value = from + (to - from) * pos; 
        circle.setAttribute(any, value);
        
        if(pct < 1) {
          requestAnimationFrame(step);
        } else {
          circle.setAttribute(any, to);
          resolve();
        }
      }
...
function drawBoard() {
    ...
    async function dropBall() {
        ...
        const circle = createCircle(x, y, BALL_RADIUS, chroma(BALL_COLOR).darken(2*Math.random() - 1).saturate(2*Math.random() - 1), chroma(BALL_COLOR).darken(2*Math.random() - 1).saturate(2*Math.random() - 1), svgElement); //*MODIFIED*
        ...
        for(let i = 0; i < NUM_LEVELS-1; i++) {
            ...
            const f = chroma.scale(['yellow', '008ae5']); //*ADDED*
            const peg = pegs[row][col]; // The peg that the ball hit
            const PEG_COLOR = f(hitCounts[row][col]/NUM_BALLS); //*ADDED*
            ...
            if(hitCounts[row][col] === 1) { // If this is the first time the peg was hit (i.e. the peg was not hit before this ball was dropped
                peg.setAttribute('fill', PEG_COLOR); // Change the color of the peg to indicate that it was hit *MODIFIED*
            } else {
                peg.setAttribute('fill', PEG_COLOR); // Change the color of the peg to indicate that it was hit *MODIFIED*
            }
        }
    ...
    async function dropBalls() {
        ...
        numBallsInput.setAttribute('disabled', true); //*ADDED*
        rightProbInput.setAttribute('disabled', true); //*ADDED*

        const dropBallPromises = []; // An array of promises for each ball that is dropped
        const NUM_BALLS = parseInt(numBallsInput.value); //*ADDED*
        ...
        numBallsInput.removeAttribute('disabled'); //*ADDED*
        rightProbInput.removeAttribute('disabled'); //*ADDED*
    }
...
async function changeHeightTo(rect, toHeight, duration) {
    ...
    await animate(rect, toHeight, duration, 'height', easeInQuad); //*ADDED*
}

// Animates the movement of a circle to a new location
async function moveCircleTo(circle, cx, cy, duration) {
    // await pause(duration);
    await Promise.all([animate(circle, cx, duration, 'cx', easeOutQuad), animate(circle, cy, duration, 'cy', easeOutQuad)]); //*ADDED*
...
numBallsInput.addEventListener('input', redrawBoard); //*ADDED*
rightProbInput.addEventListener('input', redrawBoard); //*ADDED*
